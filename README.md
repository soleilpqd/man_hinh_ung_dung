...
- `WidgetLuongManHinhXepLop`: widget luồng màn hình hiển thị widget các màn hình trong `Stack` (tức là xếp chồng lên nhau theo nhiều lớp), có áp dụng hiệu ứng chuyển động.
  ...
  - Tham số điều khiển:
    ...
    - Cấu hình `manHinh.thamSoDieuKhienWidgetLuong[WidgetLuongManHinhXepLop.kKeyThamSoHoatHinhMauNen] = ...` hoặc `dieuKhienLuong.themManHinh(manHinh: dieuKhienManHinh, thamSo: {WidgetLuongManHinhXepLop.kKeyThamSoHoatHinhMauNen: ...});` với giá trị kiểu `KieuHieuUngMauNen` để chỉ định thêm hiệu ứng chuyển màu nền của container widget màn hình.
